...
const headers = ref([
  ...
  {title: '附件', key: 'appendUrl', align: 'end'},
])
...
function downloadFiles (item) {
  window.open(item.appendUrl)
}

const selectedImage = ref('')

function showBigSizeImg (item) {
  selectedImage.value = item.appendUrl
}
            ...
            >
              <v-tabs-window-item>
                ...
                >
                  ...
                  <template #[`item.appendUrl`]="{ item }">
                    <v-btn
                      v-if="!imageList.includes(checkFileType(item.appendUrl)) && item.appendUrl"
                      rounded
                      color="info"
                      variant="outlined"
                      elevation="0"
                      @click.stop="downloadFiles(item)"
                    >
                      下载
                    </v-btn>
                    <v-img
                      v-else-if="item.appendUrl"
                      :src="item.appendUrl"
                      @click="showBigSizeImg(item)"
                    />
                  </template>
                </v-data-table>
              </v-tabs-window-item>

...
import {addAttribute,updateAttribute,getAttribute,hashAttributeName,hashAttributeWithFiles,hashAttributeWithSystem,flipAttributes} from "../../store/aaden/readFiles/Attribute"
...
async function uploadAll (url, rawFileData) {
  step.value = '开始绑定Attribute' + `<br>` + step.value
  ...
  for (const row of rawFileData) {
    ...
    nonCodeKeys.forEach((key) => {
      if (row[key].toString() !== '1') {
        ...
        needHideAttribute.push({dishId: dishInfo.dishId,agId:attributeInfo.attributeGroupId,aId:attributeInfo.id})
      }
    })
  }
  const hideReqs = []
  const allHideResults= []
  for (const hideInfo of needHideAttribute) {
    hideReqs.push(flipAttributes(url, hideInfo.dishId,hideInfo.agId,hideInfo.aId))
    if (hideReqs.length === 30) {
      // step.value = '执行一批30个属性绑定请求' + `<br>` + step.value
      try {
        const batchResults = await Promise.all(hideReqs)
        allHideResults.push(...batchResults)
        // step.value = '完成一批30个属性请求' + `<br>` + step.value
      } catch (error) {
        console.error('新增产品批次请求失败:', error)
        throw error
      }
  ...
  try {
    if (hideReqs.length > 0) {
      // step.value = '开始处理剩余的属性组更新请求' + `<br>` + step.value
      const remainingResults = await batchRequests(hideReqs, 5)
      allHideResults.push(...remainingResults)
      // step.value = '结束处理剩余的属性组更新请求' + `<br>` + step.value
    }
  } catch (e) {
    console.error(e)
